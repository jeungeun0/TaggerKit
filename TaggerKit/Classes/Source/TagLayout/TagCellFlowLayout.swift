//
//  TagCellFlowLayout.swift
//  TaggerKit
//
//  Created by JeungEun on 2022/08/29.
//

import Foundation
import UIKit

public class TagCellFlowLayout: UICollectionViewFlowLayout {
    ...
    weak var delegate: TagCellFlowLayoutDelegate?
    
    //MARK: - Init Methods
    
    public init(alignment: LayoutAlignment = .left, delegate: TagCellFlowLayoutDelegate?) {
...
private extension TagCellFlowLayout {
    
    var currentTagFrame: CGRect {
        guard let info = currentTagLayoutInfo?.layoutAttribute else { return .zero }
        var frame = info.frame
        frame.origin.x += info.bounds.width + (delegate?.tagCellFlowLayoutInteritemHorizontalSpacing(layout: self) ?? 0.0)
        return frame
    }
    ...
    func createLayoutAttributes() {
        ...
        let tagSize = delegate!.tagCellFlowLayoutTagSize(layout: self, atIndex: currentTagIndex)
    ...
    func tagCellLayoutInfo(tagIndex: Int, tagSize: CGSize) -> LayoutInfo {
        ...
        if shouldMoveTagToNextRow(tagWidth: tagSize.width) {
            ...
            (delegate?.tagCellFlowLayoutInteritemVerticalSpacing(layout: self) ?? 0.0)
